...
@Service
public class ReopenProjectHandler implements ReopenProjectUseCase {
    private final CurrentActor currentActor;
    private final ProjectLifecycleGateway lifecycle;
    private final ProjectDetailQuery details;
    private final Clock clock;

    public ReopenProjectHandler(CurrentActor currentActor, ProjectLifecycleGateway lifecycle,
    ...
    @Override
    @Transactional
    public ProjectDetailResult reopenProject(ReopenProjectCommand command) {
        ...
        if (!"ARCHIVED".equals(project.status())) {
            throw new ProjectInvalidStateException("Only ARCHIVED projects can be reopened.");
        }
        if (!lifecycle.reopenIfCurrent(command.projectId(), actor.id(), command.expectedVersion(), clock.instant())) {
