...
export class AuthService {
    ...
    async register(data: RegisterDto) {
        ...
        await this.prismaService.$transaction([
            ...
            this.outboxEventService.create(accountId, 'account.created', {
                id: accountId,
                email: data.email,
                fullName: data.fullName,
                displayName: data.displayName,
    ...
    async refresh(req: Request, res: Response) {
        ...
        return {
            data: {
                accessTokenExpiresIn: this.jwtTokenService.parseExpiresInSeconds(
                    process.env.JWT_ACCESS_EXPIRES as `${number}${'s' | 'm' | 'h' | 'd'}`,
                ),
                refreshTokenExpiresIn: this.jwtTokenService.parseExpiresInSeconds(
                    process.env.JWT_REFRESH_EXPIRES as `${number}${'s' | 'm' | 'h' | 'd'}`,
                ),
    ...
    async oauthLogin(req: Request, res: Response) {
        ...
        if (profile.provider === 'github' && !profile.email) {
            ...
            return res.redirect(`${process.env.FE_URL}/api/auth/oauth/popup-callback?error=github_email_required`);
        }
        ...
        const handoffCode = await this.tokenService.createOAuthHandoffCode(account.id, deviceInfo);
    ...
    async logout(req: Request, res: Response) {
        ...
        try {
            ...
            await this.refreshTokenService.deleteByJti(payload.jti);
        } catch {
    ...
    async getSessions(req: Request) {
        ...
        const sessions = await this.refreshTokenService.getSessions(userId);
    ...
    async logoutSession(sessionId: string, req: Request) {
        const userId = req.get('x-user-id');
        ...
        const result = await this.refreshTokenService.deleteSession(sessionId, userId);
    ...
    async logoutAll(req: Request) {
        ...
        await this.refreshTokenService.deleteAllSessions(userId);
